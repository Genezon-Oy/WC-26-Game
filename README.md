...
*   A correct prediction (1X2) awards points equal to the snapshotted odds of that outcome.
    *   *Example:* You predict a draw (`X`) on USA vs England. The match ends `1-1`. If the draw odds were locked at `3.40`, you receive **3.40 points**. An incorrect pick yields **0 points**.
...
2. Send a `POST` request to `http://localhost:3000/api/public/seed-admin` with the following JSON payload:
   ...
   ```
3. *Security Note:* This endpoint will return `403 Forbidden` if there is already one or more administrators registered in the `user_roles` table, preventing unauthorized overrides.
